/*! angular-wakanda-connector - v0.0.0 - 2014-03-04 */
(function(exports, global) {
    ...
    (function() {
        ...
        Class = function() {}, Class.extend = function(prop) {
    ...
    Date.prototype.toJSON = function(key) {
        ...
        if (d[14]) offset = 60 * Number(d[16]) + Number(d[17]), offset *= "-" == d[15] ? 1 : -1;
        offset -= date.getTimezoneOffset(), time = Number(date) + 60 * offset * 1e3, this.setTime(Number(time));
        ...
        var str = "";
        if (str += date.getUTCFullYear(), format > 1) str += "-" + zeropad(date.getUTCMonth() + 1);
        ...
        _div: "div",
        _mod: "mod"
    }, WAF.core.restConnect.defaultService = "rest", WAF.core.restConnect.standardErrors = {
        _hostnamemissing: {
            key: "1",
        ...
        this.connectionMode = connectionMode, this.fullURL = null, this.httpMethod = WAF.core.restConnect.httpMethods._get, 
        this.hostname = "", this.app = WAF.config.pattern, this.service = WAF.core.restConnect.defaultService, 
        this.resource = "", this.attributesRequested = void 0, this.keys = [], this.expand = [], 
        this.orderby = [], this.skip = null, this.top = null, this.filter = null, this.method = null, 
        this.metadata = null, this.handler = null, this.postdata = null, this.queryPlan = null, 
        this.progressInfo = null, this.distinct = false, this.refreshOnly = false, this.error = {
            key: 0,
            string: ""
            ...
            if ("" !== this.service) url += this.service + "/"; else return this.error = WAF.core.restConnect.standardErrors._servicemissing, 
            false;
            if ("" !== this.resource) if (url += this.resource + "/", this.keys.length > 0) {
                url += "(";
                for (var i = 0; i < this.keys.length; i++) if (url += this.keys[i], i < this.keys.length - 1) url += ",";
                url += ")";
            }
            var deja$ = false;
            if (this.dataURI) if (url = this.dataURI, deja$ = url.indexOf("?") !== -1, !deja$) url += "/";
            ...
            var queryString = "";
            if (this.skip) queryString += (!deja$ ? "$skip=" : "&$skip=") + this.skip, deja$ = true;
            if (this.top) queryString += (!deja$ ? "$top=" : "&$top=") + this.top, deja$ = true;
            if (this.filter) queryString += (!deja$ ? "$filter=" : "&$filter=") + encodeURIComponent("'" + this.filter + "'"), 
            deja$ = true;
            if (this.params) queryString += (!deja$ ? "$params=" : "&$params=") + encodeURIComponent("'" + JSON.stringify(this.params) + "'"), 
            deja$ = true;
            if (this.method) queryString += (!deja$ ? "$method=" : "&$method=") + this.method, 
            deja$ = true;
            if (this.asArray) queryString += (!deja$ ? "$asArray=" : "&$asArray=") + "true", 
            deja$ = true;
            if (this.metadata) queryString += (!deja$ ? "$metadata=" : "&$metadata=") + this.metadata, 
            deja$ = true;
            if (this.distinct) queryString += (!deja$ ? "$distinct=" : "&$distinct=") + this.distinct, 
            deja$ = true;
            if (null != this.findKey) queryString += (!deja$ ? "$findKey=" : "&$findKey=") + encodeURIComponent("" + this.findKey), 
            deja$ = true;
            if (this.queryPlan) queryString += !deja$ ? "$queryplan=true&querypath=true" : "&$queryplan=true&querypath=true", 
            deja$ = true;
            if (this.progressInfo) queryString += (!deja$ ? "$progressinfo=" : "&$progressinfo=") + encodeURIComponent(this.progressInfo), 
            deja$ = true;
            if (this.timeout) queryString += (!deja$ ? "$timeout=" : "&$timeout=") + this.timeout, 
            deja$ = true;
            if (this.savedQueryString) queryString += (!deja$ ? "$savedfilter=" : "&$savedfilter=") + encodeURIComponent("'" + this.savedQueryString + "'"), 
            deja$ = true;
            if (this.savedOrderby) queryString += (!deja$ ? "$savedorderby=" : "&$savedorderby=") + this.savedOrderby, 
            deja$ = true;
            if (this.refreshOnly) queryString += (!deja$ ? "$refresh=" : "&$refresh=") + this.refreshOnly, 
            deja$ = true;
            if (this.atOnce) queryString += (!deja$ ? "$atOnce=" : "&$atOnce=") + this.atOnce, 
            deja$ = true;
            if (this.retainPositions) queryString += (!deja$ ? "$retainPositions=" : "&$retainPositions=") + this.retainPositions, 
            deja$ = true;
            if (null != this.removeAtPos) queryString += (!deja$ ? "$removeFromSet=" : "&$removeFromSet=") + this.removeAtPos, 
            deja$ = true;
            if (null != this.removeReferenceOnly) queryString += (!deja$ ? "$removeRefOnly=" : "&$removeRefOnly=") + this.removeReferenceOnly, 
            deja$ = true;
            if (null != this.filterAttributes) queryString += (!deja$ ? "$attributes=" : "&$attributes=") + encodeURIComponent(this.filterAttributes), 
            deja$ = true;
            if (null != this.addToSet) queryString += (!deja$ ? "$addToSet=" : "&$addToSet=") + encodeURIComponent("'" + JSON.stringify(this.addToSet) + "'"), 
            deja$ = true;
            if (null != this.fromSelection) queryString += (!deja$ ? "$fromSel=" : "&$fromSel=") + encodeURIComponent("'" + JSON.stringify(this.fromSelection) + "'"), 
            deja$ = true;
            if (null != this.keepSelection) queryString += (!deja$ ? "$keepSel=" : "&$keepSel=") + encodeURIComponent("'" + JSON.stringify(this.keepSelection) + "'"), 
            deja$ = true;
            ...
            if (null != this.subOrderby) queryString += (!deja$ ? "$subOrderby=" : "&$subOrderby=") + encodeURIComponent(this.subOrderby), 
            deja$ = true;
            ...
            if (this.postdata && command == WAF.core.restConnect.httpMethods._get) return this.error = WAF.core.restConnect.standardErrors._wronghttpmethod, 
            false;
            if (this.http_request = WAF.core.restConnect.getXMLHttpRequest(), !this.http_request) return;
            if (this.http_request.parent = this, options && options.generateRESTRequestOnly) return url; else {
                if (null != handler) this.http_request.onreadystatechange = function() {
                    handler(this.parent);
                };
                try {
                    if (this.fullURL) url = this.fullURL;
                    if (this.http_request.open(command, url, this.connectionMode), this.postdata) this.http_request.setRequestHeader("Content-Type", "application/json");
                    if (this.http_request.setRequestHeader("If-Modified-Since", "Thu, 1 Jan 1970 00:00:00 GMT"), 
                    this.http_request.setRequestHeader("Cache-Control", "no-cache"), this.http_request.send(this.postdata), 
                    !this.connectionMode) handler(this.http_request);
                    return true;
                } catch (e) {
                    return true;
                }
        ...
        var result = null;
        if (event.userData = userData, withError) {
            var onError = options.onError;
            if (event.error = errorInfo, null != onError) result = onError(event);
        } else {
    ...
    WAF.tools.isOptionParam = function(param) {
        ...
        handleOptions = handleOptions || {}, startingFrom = startingFrom || 0;
        ...
        if (nextparam < nbparam) if (p = args[nextparam], "function" === typeof p) {
            if (res.options.onSuccess = p, ++nextparam, nextparam < nbparam) if (p = args[nextparam], 
            "function" === typeof p) if (res.options.onError = p, ++nextparam, nextparam < nbparam) if (p = args[nextparam], 
            with3funcs && "function" === typeof p) res.options.atTheEnd = p, ++nextparam;
            if (null == res.options.onError) res.options.onError = res.options.onSuccess;
        }
        var params = null;
        if (queryParams) params = [];
        do if (nextparam < nbparam) if (p = args[nextparam], !alreadyOptions && WAF.tools.isOptionParam(p)) {
            alreadyOptions = true, ++nextparam;
            var oldoptions = res.options;
            if (res.options = p, null != oldoptions.onSuccess) res.options.onSuccess = oldoptions.onSuccess;
            ...
            if (!queryParams) if (nextparam < nbparam && !noUserData) res.userData = args[nextparam], 
            ++nextparam;
        } else if (queryParams) if ("object" === typeof p && !(p instanceof Date) && !(p instanceof Array)) res.userData = p, 
        ++nextparam; else params.push(p), ++nextparam; while (queryParams && nextparam < nbparam);
        ...
        return res.nextParam = nextparam, res;
    }, WAF.EntityCache = function(options) {
        options = options || {};
        var cache = this;
        this.maxEntities = options.maxEntities || 300, this.timeOut = options.timeOut || 5 * 60 * 1e3, 
        this.entitiesByKey = {}, this.nbEntries = 0, this.curStamp = 0;
    }, WAF.EntityCache.prototype.getNextStamp = function() {
        var cache = this;
        return cache.curStamp++, cache.curStamp;
    }, WAF.EntityCache.prototype.clear = function(nbToClear) {
        var cache = this;
        if (null == nbToClear || nbToClear >= cache.nbEntries) cache.entitiesByKey = {}, 
        cache.nbEntries = 0; else {
            var all = [];
            ...
            for (var i = 0; i < nbelem; i++) {
                var entry = all[i];
                delete map[entry.key], --nbEntries;
            }
            cache.nbEntries = nbEntries, delete all;
        }
        ...
        var cache = this;
        var elem = cache.entitiesByKey[key];
        if (null != elem) elem.timeStamp = new Date(), elem.rawEntity = rawEntity, elem.stamp = cache.getNextStamp();
        ...
        options = options || {}, userData = userData;
        var dataStore = this;
        this._private = {
            owner: this,
            dataClasses: {},
        ...
        userData = userData || null, options = options || {};
        var priv = this._private;
        options.catalog = dataClassesList, options.mergeCatalog = true, priv.getCatalog(options, userData);
    }, WAF.DataStore.resolveRelatedAttribute = function() {
        if (!this.resolved) {
            var dataStore = this.owner.getDataStore();
            if (this.relatedClass = dataStore.getDataClass(this.type), null != this.relatedClass) this.resolved = true;
        }
    }, WAF.DataStore.getRelatedClassAttribute = function() {
        return this.resolve(), this.relatedClass;
        ...
        if ("entity" == methodref.applyTo) {
            entity = from, dataClass = entity.getDataClass(), request.attributesRequested = [ methodref.name ], 
            request.resource = dataClass.getName() + "(";
            var key = entity.getKey();
            if (null != key) request.resource += key;
            request.resource += ")";
        } else if ("entityCollection" == methodref.applyTo) {
            if (entityCollection = from, dataClass = entityCollection.getDataClass(), null != entityCollection._private.dataURI) request.dataURI = entityCollection._private.dataURI + "/" + methodref.name; else request.attributesRequested = [ methodref.name ], 
            request.resource = dataClass.getName(), request.filter = options.queryString;
            entityCollection._private.updateOptions(options);
        } else if ("general" == methodref.applyTo) request.resource = methodref.nameSpace, 
        request.attributesRequested = [ methodref.name ]; else dataClass = from, request.attributesRequested = [ methodref.name ], 
        request.resource = dataClass.getName();
        var pageSize = options.pageSize || 40;
        if (request.top = pageSize, request.method = "entityset", request.timeout = 300, 
        request.addToSet = options.addToSet, null != options.autoExpand) request.autoExpand = options.autoExpand;
        ...
        if (!sync) request.handler = function() {
            ...
            event.XHR = request.http_request, WAF.callHandler(null != err, err, event, options, userData);
        };
        if (generateRESTRequestOnly) result = request.go({
            generateRESTRequestOnly: true
        }); else if (request.go(), sync) {
            var successCallback, failCallback;
            if (successCallback = function() {}, failCallback = function() {}, params.length > 0) {
            ...
            if (4 != request.http_request.readyState) throw {
                ...
                var fullResult = WAF.DataStore.handleFuncResult(request, methodref, dataClass);
                if (null != fullResult.__ERROR) throw failCallback(fullResult), fullResult.__ERROR; else result = fullResult.result, 
        ...
        var func = function() {
            ...
            for (var i = 0, nb = arguments.length; i < nb; i++) {
                var p = arguments[i];
                if (!alreadyParsedOptions && WAF.tools.isOptionParam(p)) options = p, alreadyParsedOptions = true; else myargs.push(p);
            }
        ...
        userData = resOp.userData, options = resOp.options;
        ...
        if (null != options.catalog) {
            ...
            if (merge) if (classList = catResource.split(","), newlist = [], classList.forEach(function(classname) {
                if ("" != classname && null == dataStore[classname]) newlist.push(classname);
            }), 0 == newlist.length) alreadyDone = true;
        }
        if (alreadyDone) {
            var event = {
                dataStore: dataStore,
            ...
            request.resource = catResource, request.handler = function() {
                if (4 != request.http_request.readyState) return;
                var error = false;
                ...
                try {
                    ...
                    if (null != result.__ERROR) error = true, err = result.__ERROR;
                } catch (e) {
                    entitiesList = [], error = true, err = e;
                }
                for (var i = 0, nb = entitiesList.length; i < nb; i++) {
                    ...
                    if (null == dataStore[emName]) {
                        ...
                        if (priv.dataClasses[emName] = dataClass, null != collectionName) priv.dataClassesByCollectionName[collectionName] = dataClass;
                ...
                event.XHR = request.http_request, WAF.callHandler(error, err, event, options, userData);
            };
        ...
        if (priv.methods = methods, null != attributes) for (var j = 0, nbatt = attributes.length; j < nbatt; j++) {
            var att = attributes[j];
            if (att.owner = dataClass, attsByName[att.name] = att, "storage" == att.kind || "calculated" == att.kind || "alias" == att.kind || "composition" == att.kind) {
                if (att.simple = true, att.related = false, "image" == att.type || "blob" == att.type) att.simple = false;
            } else att.simple = false, att.related = true, att.resolved = false, att.relatedOne = "relatedEntity" == att.kind, 
            att.resolve = WAF.DataStore.resolveRelatedAttribute, att.getRelatedClass = WAF.DataStore.getRelatedClassAttribute, 
        ...
        if (null != methlist) for (var j = 0, nbmethod = methlist.length; j < nbmethod; j++) {
            ...
            if ("entity" == methodRef.applyTo) entityMethodRefs[methodRef.name] = methodRef, 
            entityMethods[methodRef.name] = WAF.DataStore.makeFuncCaller(methodRef); else if ("entityCollection" == methodRef.applyTo) entityCollectionMethodRefs[methodRef.name] = methodRef, 
            entityCollectionMethods[methodRef.name] = WAF.DataStore.makeFuncCaller(methodRef); else dataClassMethodRefs[methodRef.name] = methodRef, 
        ...
        request.autoExpand = options.autoExpand, request.autoSubExpand = options.autoSubExpand, 
        request.filterAttributes = options.filterAttributes, request.handler = function() {
            if (4 != request.http_request.readyState) return;
            var error = false;
            var err = null;
            var result = WAF.getRequestResult(request);
        ...
        if (null == collectionReference) {
            ...
            var resOp = WAF.tools.handleArgs(arguments, 1);
            userData = resOp.userData, options = resOp.options, options.dataURI = collectionReference.dataURI, 
            options.pageSize = options.pageSize || collectionReference.pageSize, options.autoExpand = options.autoExpand || collectionReference.autoExpand, 
            options.filterAttributes = options.filterAttributes || collectionReference.filterAttributes, 
            options.savedQuery = collectionReference.savedQuery || null, options.savedOrderby = collectionReference.savedOrderby || null;
        ...
        var dataClass = this;
        var request = new WAF.core.restConnect.restRequest(true);
        if (request.resource = dataClass.getName(), request.filter = queryString, request.skip = skip, 
        request.top = top, request.options = {
            objectRef: this,
            first: skip,
            top: top
        }, null != attributeName) request.attributesRequested = [ attributeName ];
        request.distinct = true, request.addToSet = options.addToSet, request.handler = function() {
            if (4 != request.http_request.readyState) return;
            var rawResult = WAF.getRequestResult(request);
            var withError = false;
            var err = null;
            if (null != rawResult.__ERROR) {
                err = rawResult.__ERROR, withError = true;
        ...
        var resOp = WAF.tools.handleArgs(arguments, 0);
        return userData = resOp.userData, options = resOp.options, this.query("", options, userData);
        ...
        var priv = this._private;
        var request = new WAF.core.restConnect.restRequest(true);
        request.resource = dataClass.getName(), request.autoExpand = options.autoExpand, 
        request.filterAttributes = options.filterAttributes, request.filter = queryString, 
        request.top = 1, request.orderby = options.orderby, request.params = options.params, 
        ...
        var dataClass = this;
        var request = new WAF.core.restConnect.restRequest(true);
        if (request.resource = this.getName(), request.top = options.top || null, request.skip = options.skip || null, 
        request.addToSet = options.addToSet, request.retainPositions = options.retainPositions || null, 
        request.progressInfo = options.progressInfo, request.orderby = options.orderby, 
        request.autoExpand = attributeList, request.asArray = true, options.filterQuery) request.filter = options.filterQuery, 
        ...
        var page = this;
        this.start = start, this.length = length, this.entityKeys = [];
        ...
        var savedQuery;
        if (null != dejaSet) if (savedQuery = dejaSet._private.savedQuery, null != savedQuery) {
            if (null != queryString) savedQuery = "(" + savedQuery + ") and " + queryString;
        } else saveQuery = queryString; else savedQuery = queryString;
        ...
        if (this.length = 0, options.init = true, options.createEmptyCollection) priv.ready = true, 
        queryString = "*"; else if (null != options.dataURI && "*" == options.dataURI) priv.ready = true; else if (null != options.prefetchedData) entityCollection._private.manageData(options.prefetchedData, true); else {
        ...
        var priv = this;
        return priv.curPendingStamp++, priv.pendingRequests[priv.curPendingStamp] = {
            start: start,
            length: length
        ...
        var cache = dataClass.getCache();
        if (init = init || false, init) if (entityCollection.length = rawResult.__COUNT, 
        priv.ready = true, priv.loadedElemsLength = entityCollection.length, priv.dataURI = rawResult.__ENTITYSET, 
        null != priv.autoSubExpand) priv.autoExpand = priv.autoSubExpand, priv.autoSubExpand = null;
        ...
        for (var i = 0; i < nbEnts; i++) {
            var rawEntity = arr[i];
            var xkey;
            if (null == rawEntity) cache.setEntry("", {
                __STAMP: 0
            }, timeStamp), xkey = ""; else {
                if (xkey = rawEntity.__KEY, null == xkey) xkey = "";
        ...
        var request = new WAF.core.restConnect.restRequest(true);
        if (request.entityCollection = entityCollection, request.resource = priv.dataClass.getName(), 
        null != dejaSet) if (null != dejaSet._private.dataURI) request.dataURI = dejaSet._private.dataURI, 
        request.filter = priv.queryString; else request.filter = priv.savedQuery; else request.filter = priv.queryString;
        if (request.top = top, request.skip = skip, request.attributesRequested = null, 
        request.attributesExpanded = null, request.autoExpand = priv.autoExpand, request.autoSubExpand = priv.autoSubExpand, 
        request.filterAttributes = priv.filterAttributes, request.params = priv.params, 
        priv.mustRefreshCollectionOnNextFetch) request.refreshOnly = true, priv.mustRefreshCollectionOnNextFetch = false;
        if (options.fromSelection) request.fromSelection = options.fromSelection;
        if (init) {
            request.removeAtPos = options.removeAtPos, request.removeReferenceOnly = options.removeReferenceOnly, 
            request.addToSet = options.addToSet;
            var sel = request.fromSelection;
            if (null != sel) request.fromSelection = sel.prepareToSend();
        }
        if (request.queryPlan = priv.withQueryPlan, request.progressInfo = priv.progressInfo, 
        priv.isARelatedEntityCollection) request.method = "subentityset"; else request.method = "entityset";
        if (request.timeout = options.timeout || 300, request.savedOrderby = priv.savedOrderby, 
        request.savedQueryString = priv.savedQuery, request.dataURI = request.dataURI || priv.dataURI, 
        request.dataURI && null == dejaSet) request.filter = null;
        request.orderby = options.orderby, request.subOrderby = options.subOrderby, request.keepSelection = options.keepSelection, 
        request.handler = function() {
            if (4 != request.http_request.readyState) return;
        ...
        if (null != priv.dataURI) {
            ...
            }, null != priv.savedQuery) result.savedQuery = priv.savedQuery;
        ...
        var dataClass = entityCollection.getDataClass();
        options.pageSize = options.pageSize || entityCollection._private.pageSize, options.autoExpand = options.autoExpand || entityCollection._private.autoExpand, 
        ...
        if (0 == this.length) {
            ...
            return WAF.callHandler(false, null, event, options, userData), entityCollection;
        } else {
            options.orderby = orderByString;
            var priv = entityCollection._private;
            options.dataURI = priv.dataURI;
            var dataClass = entityCollection.getDataClass();
            options.pageSize = options.pageSize || priv.pageSize, options.autoExpand = options.autoExpand || priv.autoExpand, 
            options.filterAttributes = options.filterAttributes || priv.filterAttributes, priv.updateOptions(options), 
        ...
        if (forceCollectionRefresh) priv.clearAllPendingRequest(), priv.clearCache(), priv.mustRefreshCollectionOnNextFetch = true, 
        ...
        if (pos >= lenToCheck) {
            ...
            if (subpos < priv.addedElems.length) newevent.entity = priv.addedElems[subpos], 
            WAF.callHandler(false, null, newevent, options, userData); else if (forceCollectionRefresh) WAF.callHandler(false, null, newevent, options, userData); else WAF.callHandler(true, [ {
                error: 501,
            ...
            if (null != key) {
                var cache = dataClass.getCache();
                var cacheInfo = cache.getCacheInfo(key);
                if (null != cacheInfo) executed = true, priv.gotEntity(cacheInfo, options, userData, pos); else okfetch = true, 
                key = null, priv.invalidPage(pos);
            }
            if (doNotFetch) okfetch = false;
            if (null == key && okfetch) {
                var delay, delayInfo;
                if (null != options) delay = options.delay, delayInfo = options.delayInfo; else delay = null, 
                delayInfo = null;
                if (null != delay && 0 != delay && null != delayInfo) {
                    ...
                    if (null == delayReq) {
                        ...
                        delayReq = delayInfo.addPendingRequest(0, posbefore, pos + priv.pageSize - 1), delayReq.addFetchRequest(pos, options, userData);
                                    ...
                                    onSuccess: function(event) {
                                        ...
                                        delayReq.pendingFetch.forEach(function(fetchItem) {
                                            if (fetchItem.pos >= delayInfo.top && fetchItem.pos <= delayInfo.bottom) {
                                                key = priv.getKeyByPos(fetchItem.pos), cache = dataClass.getCache();
                                                var cacheInfo = cache.getCacheInfo(key);
                    ...
                    priv.fetchData(pos, priv.pageSize, {
                        onSuccess: function(event) {
                            if (forceCollectionRefresh) priv.addedElems = [];
                            key = priv.getKeyByPos(pos), cache = dataClass.getCache();
                            var cacheInfo = cache.getCacheInfo(key);
        ...
        var priv = entityCollection._private;
        var stop = false;
        while (executed && parseData.curelem < parseData.limit && !stop) if (executed = entityCollection.getEntity(parseData.curelem, options, parseData.userData, true), 
        null != options && options.mustStopLoop) stop = true; else if (executed) parseData.curelem++; else entityCollection.getEntity(parseData.curelem, {
            onSuccess: function(event) {
        ...
        if (null != entity) this._private.addedElems.push(entity), this.length++;
    }, WAF.EntityCollection.refresh = function(options, userData) {
        options = options || {}, options.forceCollectionRefresh = true;
        var from = 0;
        if (null != options.refreshCollectionFrom) from = options.refreshCollectionFrom;
        userData = userData, this.getEntity(from, options, userData);
        ...
        var request = new WAF.core.restConnect.restRequest(true);
        if (request.entityCollection = entityCollection, request.resource = priv.dataClass.getName(), 
        request.top = options.top || priv.pageSize || 40, request.skip = options.skip || null, 
        request.params = priv.params, request.addToSet = options.addToSet, request.retainPositions = options.retainPositions || null, 
        request.progressInfo = options.progressInfo || priv.progressInfo, request.savedOrderby = priv.savedOrderby, 
        request.savedQueryString = priv.savedQuery, request.dataURI = priv.dataURI, request.orderby = options.orderby, 
        request.autoExpand = attributeList, request.asArray = true, options.filterQuery) request.filter = options.filterQuery, 
        request.params = options.params;
        request.handler = function() {
            if (4 != request.http_request.readyState) return;
        ...
        var request = new WAF.core.restConnect.restRequest(true);
        if (request.entityCollection = entityCollection, request.resource = priv.dataClass.getName(), 
        request.addToSet = options.addToSet, request.top = options.top || null, request.skip = options.skip || null, 
        request.params = priv.params, request.progressInfo = options.progressInfo || priv.progressInfo, 
        request.savedOrderby = priv.savedOrderby, request.savedQueryString = priv.savedQuery, 
        request.dataURI = priv.dataURI, request.distinct = true, null != attributeName) request.attributesRequested = [ attributeName ];
        request.handler = function() {
            if (4 != request.http_request.readyState) return;
            var rawResult = WAF.getRequestResult(request);
            var event = {
        ...
        request.params = priv.params, request.progressInfo = options.progressInfo || priv.progressInfo, 
        request.savedOrderby = priv.savedOrderby, request.savedQueryString = priv.savedQuery, 
        request.dataURI = priv.dataURI, key instanceof Date) key = key.toISO();
        request.findKey = key, request.handler = function() {
            if (4 != request.http_request.readyState) return;
            var rawResult = WAF.getRequestResult(request);
            var event = {
                entityCollection: entityCollection,
        ...
        var resOp = WAF.tools.handleArgs(arguments, 1);
        userData = resOp.userData, options = resOp.options, options.removeReferenceOnly = true, 
        ...
        if (posInSet >= priv.loadedElemsLength && posInSet < entityCollection.length) {
            ...
            if (null == entity || null == entity.getKey() || options.removeReferenceOnly) priv.addedElems.splice(subpos, 1), 
            entityCollection.length--, WAF.callHandler(false, null, resev, options, userData); else entity.remove({
                onSuccess: function(ev) {
                    priv.addedElems.splice(subpos, 1), entityCollection.length--, WAF.callHandler(false, null, resev, options, userData);
                },
                onError: function(ev) {
            ...
            options.pageSize = options.pageSize || priv.pageSize, options.autoExpand = options.autoExpand || priv.autoExpand, 
            options.filterAttributes = options.filterAttributes || priv.filterAttributes, priv.updateOptions(options), 
            options.removeAtPos = posInSet, options.savedQuery = priv.savedQuery || null, options.savedOrderby = priv.savedOrderby || null;
        ...
        priv.updateOptions(options);
        var request = new WAF.core.restConnect.restRequest(true);
        request.entityCollection = entityCollection, request.resource = priv.dataClass.getName(), 
        request.addToSet = options.addToSet, request.params = priv.params, request.progressInfo = options.progressInfo || priv.progressInfo, 
        request.savedOrderby = priv.savedOrderby, request.savedQueryString = priv.savedQuery, 
        request.dataURI = priv.dataURI, request.method = "delete", request.atOnce = options.atOnce, 
        request.handler = function() {
            if (4 != request.http_request.readyState) return;
            var rawResult = WAF.getRequestResult(request);
            var event = {
        ...
        options.pageSize = options.pageSize || priv.pageSize, options.autoExpand = options.autoExpand || priv.autoExpand, 
        options.filterAttributes = options.filterAttributes || priv.filterAttributes, priv.updateOptions(options), 
        options.filterSet = entityCollection, options.fromSelection = selection, options.savedQuery = priv.savedQuery || null, 
        options.savedOrderby = priv.savedOrderby || null;
        var subEntityCollection = new WAF.EntityCollection(dataClass, null, options, userData);
        return subEntityCollection;
    }, WAF.EntityAttributeSimple = function(entity, rawVal, att) {
        if (this.owner = entity, "date" == att.type && "string" == typeof rawVal) if (att.simpleDate) this.value = stringToSimpleDate(rawVal); else this.value = ISOToDate(rawVal); else this.value = rawVal;
        this.touched = false, this.att = att, this.getValue = WAF.EntityAttributeSimple.getValue, 
        this.setValue = WAF.EntityAttributeSimple.setValue, this.touch = WAF.EntityAttributeSimple.touch, 
        this.isTouched = WAF.EntityAttributeSimple.isTouched, this.clearTouched = WAF.EntityAttributeSimple.clearTouched, 
        this.getOldValue = WAF.EntityAttributeSimple.getOldValue, this.setRawValue = WAF.EntityAttributeSimple.setRawValue, 
        ...
        this.value = val, this.touch();
    }, WAF.EntityAttributeSimple.touch = function() {
        this.touched = true, this.owner.touch();
        ...
        if (WAF.EntityAttributeSimple.call(this, entity, rawVal, att), null != rawVal) if (null != rawVal.__deferred) this.relEntity = null, 
        this.dataURI = rawVal.__deferred.uri, this.relKey = rawVal.__deferred.__KEY; else {
            var relClass = att.getRelatedClass();
            if (null == relClass) this.relEntity = null, this.dataURI = null, this.relKey = null; else this.relEntity = new WAF.Entity(relClass, rawVal), 
            this.dataURI = null, this.relKey = null;
        } else this.relEntity = null, this.dataURI = null, this.relKey = null;
        this.setValue = WAF.EntityAttributeRelated.setValue, this.getValue = WAF.EntityAttributeRelated.getValue, 
        this.load = this.getValue, this.setRawValue = WAF.EntityAttributeRelated.setRawValue, 
        this.getRawValue = WAF.EntityAttributeRelated.getRawValue, this.getOldValue = WAF.EntityAttributeRelated.getOldValue;
        ...
        if (null == options) return this.relEntity; else {
            ...
            if (null == val.relEntity) if (null == val.dataURI && null == val.relKey) WAF.callHandler(false, null, event, options, userData); else {
                ...
                if (null == relClass) WAF.callHandler(false, null, event, options, userData); else if (null != val.relKey) relClass.getEntity(val.relKey, {
                    onSuccess: function(event) {
                        val.relEntity = event.entity, WAF.callHandler(false, null, event, options, userData);
                    },
                    onError: function(event) {
            ...
            } else event.entity = val.relEntity, WAF.callHandler(false, null, event, options, userData);
        }
    }, WAF.EntityAttributeRelated.setValue = function(relatedEntity) {
        if (void 0 === this.oldValue) this.oldValue = this.relEntity;
        if (this.relEntity = relatedEntity, this.touched = true, this.owner.touch(), this.dataURI = null, 
        null == relatedEntity) this.relKey = null; else this.relKey = relatedEntity.getKey();
    }, WAF.EntityAttributeRelated.setRawValue = function(rawVal) {
        if (null != rawVal) if (null != rawVal.__deferred) this.relEntity = null, this.dataURI = rawVal.__deferred.uri, 
        this.relKey = rawVal.__deferred.__KEY; else {
        ...
        if (WAF.EntityAttributeSimple.call(this, entity, rawVal, att), null != rawVal) if (null != rawVal.__deferred) this.relEntityCollection = null, 
        this.dataURI = rawVal.__deferred.uri; else {
            var relClass = att.getRelatedClass();
            if (null == relClass) this.relEntityCollection = null, this.dataURI = null; else this.relEntityCollection = new WAF.EntityCollection(relClass, null, {
                prefetchedData: rawVal
            }), this.dataURI = this.relEntityCollection._private.dataURI;
        } else this.relEntityCollection = null, this.dataURI = null;
        this.setValue = WAF.EntityAttributeRelatedSet.setValue, this.getValue = WAF.EntityAttributeRelatedSet.getValue, 
        this.setRawValue = WAF.EntityAttributeRelatedSet.setRawValue;
    }, WAF.EntityAttributeRelatedSet.setRawValue = function(rawVal) {
        if (null != rawVal) if (null != this.dataURI && rawVal.__deferred == this.dataURI) ; else if (null != rawVal.__deferred) this.relEntityCollection = null, 
        this.dataURI = rawVal.__deferred.uri; else {
            var relClass = this.att.getRelatedClass();
            if (null == relClass) this.relEntityCollection = null, this.dataURI = null; else this.relEntityCollection = new WAF.EntityCollection(relClass, null, {
                prefetchedData: rawVal
        ...
        if (null == options) return this.relEntityCollection; else {
            ...
            if (null != this.relEntityCollection) if (relSet = this.relEntityCollection, relSet._private.ready) event.entityCollection = relSet, 
            WAF.callHandler(false, null, event, options, userData); else {
                var funcToCall = function() {
                    if (relSet._private.ready) event.entityCollection = relSet, WAF.callHandler(false, null, event, options, userData); else setTimeout(funcToCall, 100);
                };
                ...
                if (null == relClass) WAF.callHandler(true, {
                    ...
                    options.isARelatedEntityCollection = true, options.dataURI = this.dataURI, this.relEntityCollection = new WAF.EntityCollection(relClass, null, options, userData), 
        ...
        this.touch = WAF.Entity.touch, this.getDataClass = WAF.Entity.getDataClass, this.getKey = WAF.Entity.getKey, 
        this.setStamp = WAF.Entity.setStamp, this.setKey = WAF.Entity.setKey, this.getStamp = WAF.Entity.getStamp, 
        this.callMethod = WAF.Entity.callMethod, this.isNew = WAF.Entity.isNew, this.isTouched = WAF.Entity.isTouched, 
        this.save = WAF.Entity.save, this.remove = WAF.Entity.remove, this.serverRefresh = WAF.Entity.serverRefresh;
        var isnew = false;
        if (null == rawData || null == rawData.__KEY) {
            if (this._private.key = null, this._private.stamp = 0, this._private.isNew = true, 
            isnew = true, null != rawData) this._private.touched = true;
        } else this._private.key = rawData.__KEY, this._private.stamp = rawData.__STAMP, 
        ...
        for (var e in attsByName) {
            ...
            values[e] = valAtt, entity[e] = valAtt;
        }
        ...
        var key = entity.getKey();
        if (null != key) {
            if (result.__KEY = key, result.__STAMP = entity.getStamp(), overrideStamp) result.__STAMP = -result.__STAMP;
        ...
        var resOp = WAF.tools.handleArgs(arguments, 0);
        userData = resOp.userData, options = resOp.options, options.refreshOnly = true, 
        ...
        var key = entity.getKey();
        var request = new WAF.core.restConnect.restRequest(true);
        request.resource = dataClassName + "(" + key + ")", request.method = "delete", request.handler = function() {
            if (4 != request.http_request.readyState) return;
            var error = false;
            var err = null;
            var result = WAF.getRequestResult(request);
            if (null != result.__ERROR) error = true, err = result.__ERROR; else dataClass.getCache().removeCachedEntity(key);
            var event = {
                entity: entity
            };
            WAF.callHandler(error, err, event, options, userData);
        ...
        var key = entity.getKey();
        if (null == key) request.method = "update", request.httpMethod = WAF.core.restConnect.httpMethods._post; else request.method = "update", 
        request.httpMethod = WAF.core.restConnect.httpMethods._post;
        if (request.postdata = '{ "__ENTITIES": [' + JSON.stringify(entity._private.getRESTFormat(options.overrideStamp)) + "]}", 
        request.refreshOnly = refreshOnly, null != options.autoExpand) request.autoExpand = options.autoExpand;
        if (null != options.filterAttributes) request.filterAttributes = options.filterAttributes;
        request.handler = function() {
            if (4 != request.http_request.readyState) return;
            var rawResult = WAF.getRequestResult(request);
            ...
            if (rawResult) if (rawResult.__ENTITIES) if (rawResult.__ENTITIES[0]) {
                ...
                if (rawResultRec.__ERROR) err = rawResultRec.__ERROR, withError = true; else {
                    var oldKEY = key;
                    ...
                    for (var e in attsByName) {
                        ...
                        if (null == valAtt) {
                            if (!att.related) valAtt = new WAF.EntityAttributeSimple(entity, val, att); else if (att.relatedOne) valAtt = new WAF.EntityAttributeRelated(entity, val, att); else valAtt = new WAF.EntityAttributeRelatedSet(entity, val, att);
                            entity[e] = valAtt;
                        } else if (valAtt.setRawValue(val), !refreshOnly) valAtt.clearTouched();
                    }
                    if (dataClass.getCache().replaceCachedEntity(newKEY, rawResultRec), !refreshOnly) entity._private.touched = false, 
    ...
    WAF.EntityCollection.prototype.query = WAF.EntityCollection.query, WAF.EntityCollection.prototype.getEntity = WAF.EntityCollection.getEntity, 
    WAF.EntityCollection.prototype.getEntities = WAF.EntityCollection.getEntities, WAF.EntityCollection.prototype.callMethod = WAF.EntityCollection.callMethod, 
    WAF.EntityCollection.prototype.each = WAF.EntityCollection.forEach, WAF.EntityCollection.prototype.forEach = WAF.EntityCollection.forEach, 
    WAF.EntityCollection.prototype.add = WAF.EntityCollection.add, WAF.EntityCollection.prototype.orderBy = WAF.EntityCollection.orderBy, 
    WAF.EntityCollection.prototype.toArray = WAF.EntityCollection.toArray, WAF.EntityCollection.prototype.distinctValues = WAF.EntityCollection.distinctValues, 
    WAF.EntityCollection.prototype.findKey = WAF.EntityCollection.findKey, WAF.EntityCollection.prototype.removeEntity = WAF.EntityCollection.removeEntity, 
    ...
    WAF.EntityCollection.prototype.refresh = WAF.EntityCollection.refresh, WAF.DataClass.prototype.distinctValues = WAF.DataClass.distinctValues, 
    WAF.DataClass.prototype.all = WAF.DataClass.allEntities, WAF.DataClass.prototype.allEntities = WAF.DataClass.allEntities, 
    WAF.DataClass.prototype.query = WAF.DataClass.query, WAF.DataClass.prototype.find = WAF.DataClass.find, 
    WAF.DataClass.prototype.getAttributeByName = WAF.DataClass.getAttributeByName, WAF.DataClass.prototype.getAttributes = WAF.DataClass.getAttributes, 
    WAF.DataClass.prototype.getMethodList = WAF.DataClass.getMethodList, WAF.DataClass.prototype.getCache = WAF.DataClass.getCache, 
    WAF.DataClass.prototype.getCacheSize = WAF.DataClass.getCacheSize, WAF.DataClass.prototype.setCacheSize = WAF.DataClass.setCacheSize, 
    WAF.DataClass.prototype.clearCache = WAF.DataClass.clearCache, WAF.DataClass.prototype.newEntity = WAF.DataClass.newEntity, 
    WAF.DataClass.prototype.getEntity = WAF.DataClass.getEntity, WAF.DataClass.prototype.getEntityByURI = WAF.DataClass.getEntityByURI, 
    WAF.DataClass.prototype.getDataStore = WAF.DataClass.getDataStore, WAF.DataClass.prototype.getCollectionName = WAF.DataClass.getCollectionName, 
    WAF.DataClass.prototype.getDefaultTopSize = WAF.DataClass.getDefaultTopSize, WAF.DataClass.prototype.getName = WAF.DataClass.getName, 
    WAF.DataClass.prototype.newCollection = WAF.DataClass.newCollection, WAF.DataClass.prototype.toArray = WAF.DataClass.toArray, 
    WAF.DataClass.prototype.callMethod = WAF.DataStore.callMethod, WAF.directory = WAF.directory || {}, 
    WAF.directory.login = WAF.DataStore.makeFuncCaller({
        name: "login",
        applyTo: "general",
        nameSpace: "$directory"
    }), WAF.directory.loginByPassword = WAF.directory.login, WAF.directory.logout = WAF.DataStore.makeFuncCaller({
        name: "logout",
        applyTo: "general",
        nameSpace: "$directory"
        ...
        applyTo: "general",
        nameSpace: "$directory"
    }), WAF.dsExport = WAF.dsExport || {}, WAF.dsExport.exportData = WAF.DataStore.makeFuncCaller({
        name: "exportData",
        applyTo: "general",
        nameSpace: "$impexp"
    });
    ...
    wakConnectorModule.factory("wakConnectorService", [ "$q", "$rootScope", function($q, $rootScope) {
        ...
        var init = function(catalog) {
            ...
            if (null === ds) new WAF.DataStore({
                onSuccess: function(event) {
                    ds = event.dataStore, prepare.wafDatastore(ds), prepare.wafDataClasses(ds), console.log(">wakConnectorService init > success", event, "ds", ds), 
                    deferred.resolve(ds);
                },
                onError: function(event) {
                    ds = null, console.error(">wakConnectorService init > error", event), deferred.reject(event);
                },
                catalog: catalog
        ...
        var prepare = {
            ...
            wafDataClasses: function(dataStore) {
                var dataClassName;
                WAF.DataClass.prototype.$find = $$find, WAF.DataClass.prototype.$findOne = $$findOne, 
                WAF.DataClass.prototype.$create = $$create, console.group("prepare.wafDataClasses()", dataStore);
                for (dataClassName in dataStore) if (dataStore.hasOwnProperty(dataClassName) && "_private" !== dataClassName && false === /^\$.*/.test(dataClassName)) console.group("DataClass[%s]", dataStore[dataClassName]._private.className, dataStore[dataClassName]), 
                prepare.wafDataClassAddMetas(dataStore[dataClassName]), prepare.wafDataClassCreateNgWakEntityClasses(dataStore[dataClassName]), 
                console.groupEnd();
                console.groupEnd();
            },
            wafDataClassAddMetas: function(dataClass) {
                ...
                attributes = dataClass._private.attributesByName, dataClass.$attr = function(attrName) {
                ...
                }, dataClass.$name = dataClass._private.className, dataClass.$collectionName = dataClass._private.collectionName;
            },
            wafDataClassCreateNgWakEntityClasses: function(dataClass) {
                var proto;
                proto = prepareHelpers.createUserDefinedEntityMethods(dataClass), NgWakEntityClasses[dataClass._private.className] = NgWakEntityAbstract.extend(proto), 
        ...
        var prepareHelpers = {
            createUserDefinedEntityMethods: function(dataClass) {
                ...
                for (methodName in dataClass._private.entityMethods) if (dataClass._private.entityMethods.hasOwnProperty(methodName)) proto[methodName + "Sync"] = function() {
            ...
            createUserDefinedEntityCollectionMethods: function(dataClass) {
                var methodName, proto = {};
                for (methodName in dataClass._private.entityCollectionMethods) if (dataClass._private.entityCollectionMethods.hasOwnProperty(methodName)) proto[methodName + "Sync"] = function() {
            ...
            wakandaUserDefinedMethodToPromisableMethods: function(proto, methodName, method) {
                proto[methodName] = function() {
                    ...
                    if (this instanceof NgWakEntityAbstract) mode = "$_entity"; else mode = "$_collection";
                    ...
                    deferred = $q.defer();
                    var that = this;
                    return wakOptions.onSuccess = function(event) {
                        rootScopeSafeApply(function() {
                            if (console.log("userMethods.onSuccess", "event", event), "$_entity" === mode) that.$syncEntityToPojo(); else ;
                            deferred.resolve(event);
                        });
                    }, wakOptions.onError = function(error) {
                        rootScopeSafeApply(function() {
                            console.error("userMethods.onError", "error", error), deferred.reject(error);
                        });
                    }, thatArguments.unshift(wakOptions), method.apply(this[mode], thatArguments), deferred.promise;
                };
            }
        };
        var transform = {
            queryEventToNgWakEntityCollection: function(event, onlyOne) {
                ...
                if (parsedXhrResponse = JSON.parse(event.XHR.response), rawEntities = parsedXhrResponse.__ENTITIES, 
                ...
                true !== onlyOne) {
                    ...
                    result.$_collection = event.result, transform.addFrameworkMethodsToResult(result);
                } else if (1 === result.length) result = result[0]; else result = null;
                return event.result = result, console.log("after transform.queryEventToNgWakEntityCollection", "event", event), 
                event;
            },
            jsonResponseToNgWakEntityCollection: function(dataClass, xhrResponse) {
                var ngWakEntityCollection = [];
                return console.log("dataClass", dataClass), xhrResponse.map(function(pojo) {
            ...
            fetchEventToNgWakEntityCollection: function(event) {
                ...
                console.log("transform.fetchEventToNgWakEntityCollection", event), event.entities.forEach(function(entity, index) {
                    result.push(dataClass.$create(entity));
                }), console.log("transformFetchEvent", "result", result), event.result = result;
            },
            asyncResult: function(data, result, promise) {
                var collectionMethods, i;
                if (data instanceof Array) {
                    if (collectionMethods = data.$_collection.getDataClass().$collectionMethods(), result.length = 0, 
                    angular.forEach(data, function(item) {
                        result.push(item);
                    }), collectionMethods.length > 0) for (i = 0; i < collectionMethods.length; i++) result[collectionMethods[i]] = data[collectionMethods[i]];
                    result.$_collection = data.$_collection, transform.addFrameworkMethodsToResult(result), 
                    result.$promise = promise;
                } else helpers.shallowClearAndCopy(data, result), result.$promise = promise;
            },
            addFrameworkMethodsToResult: function(result) {
                result.$fetch = $$fetch, result.$add = $$add, result.$more = $$more, result.$nextPage = $$nextPage, 
                result.$prevPage = $$prevPage, result.$totalCount = result.$_collection.length;
            }
        };
        var $$create = function(pojo) {
            var dataClassName = this._private.className, ngWakEntity, entity, key, attributes;
            if (ngWakEntity = new NgWakEntityClasses[dataClassName](), pojo instanceof WAF.Entity) reccursiveFillNgWakEntityFromEntity(pojo, ngWakEntity, this); else reccursiveFillNgWakEntityFromEntity(pojo, ngWakEntity, this);
            return ngWakEntity;
        };
        var reccursiveFillNgWakEntityFromEntity = function(entity, ngWakEntityNestedObject, currentDataClass) {
            var key, attributes = currentDataClass.$attr(), isEntityWafEntity = entity instanceof WAF.Entity;
            if (false === isEntityWafEntity) ngWakEntityNestedObject.$_entity = new WAF.Entity(currentDataClass, entity); else ngWakEntityNestedObject.$_entity = entity;
            ...
            for (key in attributes) if ("storage" === attributes[key].kind) ngWakEntityNestedObject[key] = isEntityWafEntity ? entity[key].getValue() : entity[key]; else if ("relatedEntities" === attributes[key].kind) console.warn("relatedEntities", key, isEntityWafEntity ? entity[key].getRawValue() : entity[key]), 
            ngWakEntityNestedObject[key] = isEntityWafEntity ? entity[key].getRawValue() : entity[key]; else if ("relatedEntity" === attributes[key].kind) if (console.log("REL ENTITY - kind", isEntityWafEntity, attributes[key].kind, ds[currentDataClass.$name].$attr(key).type), 
            ...
            isEntityWafEntity) reccursiveFillNgWakEntityFromEntity(entity[key].relEntity, ngWakEntityNestedObject[key], entity[key].relEntity.getDataClass()); else console.log("NEXT dataClass", ds[ds[currentDataClass.$name].$attr(key).type]), 
        ...
        var updateQueryInfos = function(resultSet, pageSize, start, filter) {
            ...
            resultSet.$query.pageSize = pageSize, resultSet.$query.start = start, resultSet.$query.filter = filter ? filter : resultSet.$query.filter;
        };
        var $$fetch = function(options, mode) {
            var deferred, wakOptions = {}, that = this, skip, top;
            if (mode = "undefined" === typeof mode || "replace" === mode ? "replace" : mode, 
            !options || "object" !== typeof options) throw new Error("Please pass an object as options");
            if ("undefined" === typeof options.start) throw new Error("Please specify a start index");
            ...
            if (options.pageSize = options.pageSize || this.$query.pageSize, "undefined" !== typeof options.start) skip = options.start;
            if ("undefined" !== typeof options.pageSize) top = options.pageSize;
            if (options.params) wakOptions.params = options.params;
            console.log(wakOptions), deferred = $q.defer();
            var that = this;
            return rootScopeSafeApply(function() {
                that.$fetching = true;
            }), wakOptions.onSuccess = function(event) {
                rootScopeSafeApply(function() {
                    if (console.log("onSuccess", "originalEvent", event), transform.fetchEventToNgWakEntityCollection(event), 
                    "replace" === mode) {
                        that.length = 0;
                        for (var i = 0; i < event.result.length; i++) that[i] = event.result[i];
                    } else if ("append" === mode) for (var i = 0; i < event.result.length; i++) console.log(event.result[i]), 
                    that.push(event.result[i]);
                    updateQueryInfos(that, options.pageSize || that.$_collection._private.pageSize, skip), 
                    console.log("onSuccess", "processedEvent", event), that.$fetching = false, deferred.resolve(event);
                });
            }, wakOptions.onError = function(event) {
                rootScopeSafeApply(function() {
                    console.error("onError", event), that.$fetching = false, deferred.reject(event);
                });
            }, this.$_collection.getEntities(skip, top, wakOptions), deferred.promise;
        };
        var $$more = function() {
            var start, pageSize, deferred;
            if (start = this.$query.start + this.$query.pageSize, pageSize = this.$query.pageSize, 
            start > this.$totalCount) return deferred = new $q.defer(), deferred.resolve({
                noMore: true
            }), deferred.promise; else return this.$fetch({
                start: start,
                pageSize: pageSize
            }, "append");
        };
        ...
        var $$prevPage = function() {
            var start, pageSize, deferred;
            if (start = this.$query.start - this.$query.pageSize, pageSize = this.$query.pageSize, 
            start < 0) return deferred = new $q.defer(), deferred.resolve({
                noMore: true
            }), deferred.promise; else return this.$fetch({
                start: start,
                pageSize: pageSize
        ...
        var $$find = function(options) {
            ...
            if (onlyOne = !!options.onlyOne, onlyOne) result = new NgWakEntityClasses[this.$name](); else result = [];
            return deferred = $q.defer(), result.$promise = deferred.promise, rootScopeSafeApply(function() {
                result.$fetching = true;
            }), console.log("RESULT", result), wakOptions.onSuccess = function(event) {
                rootScopeSafeApply(function() {
                    if (console.log("onSuccess", "originalEvent", event), transform.queryEventToNgWakEntityCollection(event, onlyOne), 
                    transform.asyncResult(event.result, result, deferred.promise), false === onlyOne) updateQueryInfos(result, result.$_collection._private.pageSize, 0, query);
                    console.log("onSuccess", "processedEvent", event, result.$_collection ? result.$_collection : result.$_entity), 
                    result.$fetching = false, deferred.resolve(event);
                });
            }, wakOptions.onError = function(event) {
                rootScopeSafeApply(function() {
                    console.error("onError", event), result.$fetching = false, deferred.reject(event);
                });
            }, options = null, this.query(query, wakOptions), result;
        };
        ...
        var NgWakEntityAbstractPrototype = {
            $save: function() {
                ...
                return this.$syncPojoToEntity(), deferred = $q.defer(), wakOptions.onSuccess = function(event) {
                    rootScopeSafeApply(function() {
                        console.log("save.onSuccess", "event", event), that.$syncEntityToPojo(), deferred.resolve(event);
                    });
                }, wakOptions.onError = function(error) {
                    rootScopeSafeApply(function() {
                        console.error("save.onError", "error", error), deferred.reject(error);
                    });
                }, this.$_entity.save(wakOptions), deferred.promise;
            },
            $remove: function() {
            ...
            $syncEntityToPojo: function() {
                var pojo = this, key;
                if (pojo.$_entity && pojo.$_entity._private && pojo.$_entity._private.values) for (key in pojo.$_entity._private.values) if (console.log(key, pojo.$_entity._private.values[key]), 
                pojo.$_entity[key].getValue() !== pojo[key] && pojo.$_entity[key] instanceof WAF.EntityAttributeSimple) pojo[key] = pojo.$_entity[key].getValue();
