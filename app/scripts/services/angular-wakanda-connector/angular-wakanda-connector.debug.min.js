    ...
    wakConnectorModule.factory("wakConnectorService", [ "$q", "$rootScope", function($q, $rootScope) {
        ...
        var prepare = {
            ...
            wafDataClassAddMetas: function(dataClass) {
                ...
                for (methodName in dataClass._private.dataClassMethods) if (dataClass._private.dataClassMethods.hasOwnProperty(methodName)) dataClassMethods.push(methodName);
                for (methodName in dataClass._private.entityCollectionMethods) if (dataClass._private.entityCollectionMethods.hasOwnProperty(methodName)) collectionMethods.push(methodName);
                for (methodName in dataClass._private.entityMethods) if (dataClass._private.entityMethods.hasOwnProperty(methodName)) entityMethods.push(methodName);
        ...
        var prepareHelpers = {
            ...
            wakandaUserDefinedEntityMethodToPromisableMethods: function(proto, methodName, method) {
                proto[methodName] = function() {
                    var thatArguments = [], wakOptions = {}, deferred;
                    ...
                    return this.$syncPojoToEntity(), deferred = $q.defer(), wakOptions.onSuccess = function(event) {
                        rootScopeSafeApply(function() {
                            console.log("userMethods.onSuccess", "event", event), this.$syncEntityToPojo(), 
                    ...
                    }, thatArguments.unshift(wakOptions), method.apply(this.$_entity, thatArguments), 
                    deferred.promise;
                };
            }
        };
        var transform = {
            queryEventToNgWakEntityCollection: function(event, onlyOne) {
                ...
                true !== onlyOne) result._collection = event.result, result.$fetch = $$fetch, result.$add = $$add; else if (1 === result.length) result = result[0]; else result = null;
        ...
        var wakToAngular = {
            transformFetchEvent: function(event, mode) {
                var result;
                console.log("EVENT", event), result = event.entities, result.forEach(function(entity, index) {
                    var pojo = {};
                    pojo.$_entity = entity, wakToAngular.addFrameworkMethodsToPojo(pojo), wakToAngular.addUserDefinedEntityMethodsToPojo(pojo), 
                    pojo.$syncEntityToPojo(), result[index] = pojo;
                }), console.log("transformFetchEvent", "result", result), event.result = result;
            },
            ...
            addFrameworkMethodsToPojo: function(pojo) {
                pojo.$save = $WakEntityMethods.$$save, pojo.$remove = $WakEntityMethods.$$remove, 
                pojo.$syncPojoToEntity = $WakEntityMethods.$$syncPojoToEntity, pojo.$syncEntityToPojo = $WakEntityMethods.$$syncEntityToPojo;
            },
            addUserDefinedEntityMethodsToPojo: function(pojo) {
                var key;
                if (pojo.$_entity && pojo.$_entity._private && pojo.$_entity._private.methods) for (key in pojo.$_entity._private.methods) if (pojo.$_entity._private.methods.hasOwnProperty(key)) pojo[key + "Sync"] = function() {
            ...
            wakandaUserDefinedEntityMethodToPromisableMethods: function(pojo, methodName, method) {
                pojo[methodName] = function() {
                    var thatArguments = [], wakOptions = {}, deferred;
                    if (arguments.length > 0) for (var i = 0; i < arguments.length; i++) thatArguments.push(arguments[i]);
                    return pojo.$syncPojoToEntity(), deferred = $q.defer(), wakOptions.onSuccess = function(event) {
                        rootScopeSafeApply(function() {
                            console.log("userMethods.onSuccess", "event", event), pojo.$syncEntityToPojo(), 
                            deferred.resolve(event);
                        });
                    }, wakOptions.onError = function(error) {
                        rootScopeSafeApply(function() {
                    ...
                    }, thatArguments.unshift(wakOptions), method.apply(pojo.$_entity, thatArguments), 
                    deferred.promise;
                };
            }
        };
        ...
        var $$create = function(pojo) {
            ...
            if (ngWakEntity = new NgWakEntityClasses[dataClassName](), pojo instanceof WAF.Entity) entity = pojo, 
            reccursiveFillNgWakEntityFromEntity(entity, ngWakEntity); else {
                entity = this.newEntity();
                for (key in pojo) if (pojo.hasOwnProperty(key) && "__KEY" === key) ngWakEntity[key] = pojo[key], 
                entity.setKey(pojo[key]); else if (pojo.hasOwnProperty(key) && "__STAMP" === key) ngWakEntity[key] = pojo[key], 
                entity.setStamp(pojo[key]); else if (pojo.hasOwnProperty(key) && entity.hasOwnProperty(key)) if (ngWakEntity[key] = pojo[key], 
                entity[key] instanceof WAF.EntityAttributeSimple) entity[key].setValue(pojo[key]);
            }
            return ngWakEntity.$_entity = entity, ngWakEntity;
        };
        var reccursiveFillNgWakEntityFromEntity = function(entity, ngWakEntityNestedObject) {
            var key, attributes = entity.getDataClass().$attr();
            ngWakEntityNestedObject.__KEY = entity.getKey(), ngWakEntityNestedObject.__STAMP = entity.getStamp();
            for (key in attributes) if ("storage" === attributes[key].kind) ngWakEntityNestedObject[key] = entity[key].getValue(); else if ("relatedEntities" === attributes[key].kind) ngWakEntityNestedObject[key] = entity[key].getRawValue(); else if (entity[key].relEntity) ngWakEntityNestedObject[key] = {}, 
            reccursiveFillNgWakEntityFromEntity(entity[key].relEntity, ngWakEntityNestedObject[key]);
        };
        var $$fetch = function(skip, top, mode) {
            var deferred, wakOptions = {}, that = this;
            return mode = "undefined" === typeof mode || "replace" ? "replace" : mode, deferred = $q.defer(), 
            wakOptions.onSuccess = function(event) {
                rootScopeSafeApply(function() {
                    console.log("onSuccess", "originalEvent", event), transform.fetchEventToNgWakEntityCollection(event), 
        ...
        var $$find = function(options) {
            ...
            return deferred = $q.defer(), onlyOne = options.onlyOne, wakOptions.onSuccess = function(event) {
                rootScopeSafeApply(function() {
            ...
            }, options = null, this.query(query, wakOptions), deferred.promise;
        };
