(function(exports, global) {
    ...
    wakConnectorModule.factory("wakConnectorService", [ "$q", "$rootScope", function($q, $rootScope) {
        var ds = null, NgWakEntityClasses = {}, NgWakEntityCollectionClasses = {};
        ...
        var prepare = {
            ...
            wafDataClassAddMetas: function(dataClass) {
                var methodName, dataClassMethods = [], collectionMethods = [], entityMethods = [];
        ...
        var wakToAngular = {
            transformQueryEvent: function(event, onlyOne) {
                var result, parsedXHRResponse;
                if (parsedXHRResponse = JSON.parse(event.XHR.response), result = parsedXHRResponse.__ENTITIES, 
                result.map(function(pojo) {
                    return pojo.$_entity = new WAF.Entity(event.result.getDataClass(), pojo), wakToAngular.addFrameworkMethodsToPojo(pojo), 
                    wakToAngular.addUserDefinedEntityMethodsToPojo(pojo), pojo;
                }), true !== onlyOne) result._collection = event.result, result.$fetch = $$fetch, 
                result.$add = $$add; else if (1 === result.length) result = result[0]; else result = null;
                return event.result = result, console.log("after transformQueryEvent", "event", event), 
                event;
            },
        ...
        var $$create = function(pojo) {
            ...
            for (var key in pojo) if (pojo.hasOwnProperty(key)) entity[key].setValue(pojo[key]);
            return pojo.$_entity = entity, wakToAngular.addFrameworkMethodsToPojo(pojo), wakToAngular.addUserDefinedEntityMethodsToPojo(pojo), 
            pojo;
        };
        var $$fetch = function(skip, top, mode) {
            ...
            wakOptions.onSuccess = function(event) {
                rootScopeSafeApply(function() {
                    console.log("onSuccess", "originalEvent", event), wakToAngular.transformFetchEvent(event), 
        ...
        var $$find = function(options) {
            ...
            return deferred = $q.defer(), onlyOne = options.onlyOne, wakOptions.onSuccess = function(event) {
                rootScopeSafeApply(function() {
                    console.log("onSuccess", "originalEvent", event), wakToAngular.transformQueryEvent(event, onlyOne), 
                    console.log("onSuccess", "processedEvent", event), deferred.resolve(event);
                });
            }, wakOptions.onError = function(event) {
                rootScopeSafeApply(function() {
