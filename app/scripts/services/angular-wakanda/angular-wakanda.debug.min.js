/*! angular-wakanda - v0.4.2 - 2015-04-01 */
(function(exports, global) {
    ...
    wakanda.factory("$wakanda", [ "$q", "$rootScope", "$http", function($q, $rootScope, $http) {
        ...
        var $$create = function(pojo) {
            var dataClassName = this.getName(), ngWakEntity, wafEntity;
        ...
        var $$fetch = function(options, mode) {
            ...
            wakOptions.onSuccess = function(event) {
                rootScopeSafeApply(function() {
                    if ("replace" === mode) {
                        that.length = 0;
                        for (var i = 0; i < event.entities.length; i++) that[i] = event.result.getDataClass().$refCache.getCachedNgWakEntity(event.entities[i]);
                    } else if ("append" === mode) for (var i = 0; i < event.entities.length; i++) that.push(event.result.getDataClass().$refCache.getCachedNgWakEntity(event.entities[i]));
        ...
        var NgWakEntityAbstractPrototype = {
            init: function() {
                ...
                Object.defineProperty(this, "$_tempUUID", {
                    ...
                    configurable: false,
                    get: function() {
                        if (this.$_entity) return this.$_entity.$_tempUUID;
                    },
                    set: function(newValue) {
                        if (this.$_entity) return this.$_entity.$_tempUUID = newValue;
                    }
                });
                dataClass.getAttributes().forEach(function(attr) {
                    if ("relatedEntity" === attr.kind) Object.defineProperty(this, attr.name, {
                        ...
                        configurable: true,
                        get: function() {
                            if (this.$_entity) if (this.$_entity[attr.name] && this.$_entity[attr.name].relEntity && "undefined" !== typeof this.$_entity[attr.name].relEntity.getKey()) return ds[this.$_entity[attr.name].relEntity.getDataClass().getName()].$refCache.getCachedNgWakEntity(this.$_entity[attr.name].relEntity); else if (this.$_entity[attr.name] && null === this.$_entity[attr.name].relEntity && "undefined" !== typeof this.$_entity[attr.name].relKey) {
                                ...
                                if (cachedEntity) return cachedEntity; else return ds[attr.path].$refCache.getCachedDummyNgWakEntity(this.$_entity[attr.name].relKey);
                            } else ;
                        },
                    ...
                    }); else if ("relatedEntities" === attr.kind) ; else if ("calculated" === attr.kind || "alias" === attr.kind) Object.defineProperty(this, attr.name, {
                        enumerable: true,
                        configurable: true,
                        get: function() {
        ...
        NgWakEntityCache.prototype.setSize = function(size) {
            if ("number" !== typeof size || size < DEFAULT_CACHE_SIZE) size = DEFAULT_CACHE_SIZE;
        ...
        NgWakEntityCache.prototype.setEntry = function(ngWakEntity) {
            ...
            if (null !== key) {
                ...
                if (!this.entitiesByKey[key]) this.nbEntries++;
        ...
        NgWakEntityCache.prototype.getCachedDummyNgWakEntity = function(key) {
            var ngWakEntity;
            if (this.getDataClass().$refCache.getCacheInfo(key)) ngWakEntity = this.getDataClass().$refCache.getCacheInfo(key); else ngWakEntity = new (NgWakEntityClasses[this.getDataClass().$name])(this.getDataClass(), key);
            this.setEntry(ngWakEntity);
            return ngWakEntity;
        };
        NgWakEntityCache.prototype.getCachedNgWakEntity = function(wafEntity) {
            var ngWakEntity;
            if (wafEntity.isNew() && this.entitiesByKey[wafEntity.$_tempUUID]) return this.entitiesByKey[wafEntity.$_tempUUID]; else if (this.entitiesByKey[wafEntity.getKey()]) return this.entitiesByKey[wafEntity.getKey()]; else {
                ngWakEntity = new (NgWakEntityClasses[wafEntity.getDataClass().getName()])(wafEntity);
                if (wafEntity.getDataClass().$_relatedAttributes && wafEntity.getDataClass().$_relatedAttributes.length > 0) wafEntity.getDataClass().$_relatedAttributes.forEach(function(attr) {
                    if ("relatedEntity" === attr.kind) {
                        var nestedEntity;
                        if (wafEntity[attr.name] && wafEntity[attr.name].relEntity) nestedEntity = ds[wafEntity[attr.name].relEntity.getDataClass().getName()].$refCache.getCachedNgWakEntity(wafEntity[attr.name].relEntity); else if (wafEntity[attr.name] && wafEntity[attr.name].relKey) nestedEntity = ds[wafEntity[attr.name].att.getRelatedClass().getName()].$refCache.getCacheInfo(wafEntity[attr.name].relKey);
                        return nestedEntity;
                    } else if ("relatedEntities" === attr.kind) console.warn("getCachedNgWakEntity not on relatedEntities", attr.name, wafEntity[attr.name]);
                }.bind(this));
                this.setEntry(ngWakEntity);
                return ngWakEntity;
            }
